...
import { 
  ...
  CheckCircle2, AlertCircle, FileText, DollarSign, Calendar, Info
} from 'lucide-react';
...
export default function StudentLibraryPage() {
  ...
  const fetchLibraryData = async () => {
    try {
      setLoading(true);
      const res = await api.get('/library/logs/student_library/');
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Track borrowed books, visitor logs, and library fines/fees.
          </p>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 space-y-6">
          ...
          {loading ? (
            <div className="py-12 text-center text-slate-500 font-semibold text-sm">
              Loading borrowed books history...
            </div>
          ) : books.length === 0 ? (
